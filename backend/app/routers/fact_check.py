...
from ..schemas.fact_check import FactCheckRequest, FactCheckResponse, FactCheckItem
...
async def fact_check_page(request: FactCheckRequest, current_user: dict = Depends(lambda: None)):
    ...
    try:
        ...
        return FactCheckResponse(
            ...
            verified_claims=len([r for r in fact_check_results if r.verdict == "true"]),
            unverified_claims=len([r for r in fact_check_results if r.verdict == "false"]),
            inconclusive_claims=len([r for r in fact_check_results if r.verdict == "unverified"])
        )
